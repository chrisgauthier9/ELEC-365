...
def buildTriangles(slice0, slice1):
    ...
    # Indices for storing indices of closest pairs
    slice0Index = -1
    slice1Index = -1
    smallestDistance = -1  # int()

    # Nested for loops to go through slices, calculate the distance and store the indicies of closest pair
    for i in range(len(slice0.verts)):
        for j in range(len(slice1.verts)):
            difference = subtract(slice0.verts[i].coords, slice1.verts[j].coords)
            for num in range(len(difference)):
                difference[num] = difference[num] ** 2
            distance = sum(int(num) for num in difference) ** (1 / 2)
            if smallestDistance == -1 or distance < smallestDistance:
    ...
    # Create cyclic lists of of both slices starting at closest pair
    slice0Cyclic = []
    slice0Cyclic.extend(slice0.verts[slice0Index:])
    slice0Cyclic.extend(slice0.verts[:slice0Index + 1])

    slice1Cyclic = []
    slice1Cyclic.extend(slice1.verts[slice1Index:])
    slice1Cyclic.extend(slice1.verts[:slice1Index + 1])
    ...
    minArea = [[0 for i in range(len(slice0Cyclic))] for j in range(len(slice1Cyclic))]
    minDir = [['.' for i in range(len(slice0Cyclic))] for j in range(len(slice1Cyclic))]
    ...
    for i in range(1, len(slice0Cyclic)):
        minArea[0][i] = round(triangleArea(slice1Cyclic[0].coords, slice0Cyclic[i].coords, slice0Cyclic[i - 1].coords) + minArea[0][i - 1], 1)
        minDir[0][i] = '-'
    ...
    for j in range(1, len(slice1Cyclic)):
        minArea[j][0] = round(triangleArea(slice0Cyclic[0].coords, slice1Cyclic[j].coords, slice1Cyclic[j - 1].coords) + minArea[j - 1][0], 1)
        minDir[j][0] = '|'
    ...
    for j in range(1, len(slice1Cyclic)):
        for i in range(1, len(slice0Cyclic)):
            fromCol = round(triangleArea(slice0Cyclic[i].coords, slice1Cyclic[j].coords, slice1Cyclic[j - 1].coords) + minArea[j - 1][i], 1)
            fromRow = round(triangleArea(slice1Cyclic[j].coords, slice0Cyclic[i].coords, slice0Cyclic[i - 1].coords) + minArea[j][i - 1], 1)
            if fromRow > fromCol:
                minArea[j][i] = fromCol
                minDir[j][i] = '|'
            else:
    ...
    # for x in range(len(slice0Cyclic)):
    #     for y in range(len(slice1Cyclic)):
    #         print(minArea[x][y], ' ', end='')
    #         print(minDir[x][y], '   ', end='')
    #     print('')
    ...
    i = len(slice0Cyclic) - 1
    j = len(slice1Cyclic) - 1
    while i > - 1 and j > -1:
        if minDir[j][i] == '-':
            verts = [slice0Cyclic[i], slice1Cyclic[j], slice0Cyclic[i - 1]]
            tri = Triangle(verts)
            triangles.append(tri)
            i -= 1
        else:
            verts = [slice0Cyclic[i], slice1Cyclic[j], slice1Cyclic[j - 1]]
            tri = Triangle(verts)
            triangles.append(tri)
            j -= 1
...
def triangleArea(v0, v1, v2):
    ...
    return 0.5 * length(crossProduct(a, b))
